  ...
  return (
    <div>
      <div className={classes.dayGrid}>
        {days.map((day, index) => (
          <p key={index} className={`${alegreya.className} ${(index === dayToday && year === currentYear && month === currentMonth) ? classes.daysBold : classes.days}`}>{day}</p>
        ))}
      </div>
      <div className={classes.monthGrid}>
        {monthDates.map((date, index) => (
          <div className=
          {`${date === null ? classes.monthNull 
          : (date === dateToday && year === currentYear && month === currentMonth) ? classes.bold 
          : classes.month}`} 
          key={index}>
            <p className={alegreya.className}>{date}</p>
          </div>
        ))}
      </div>
    </div>
